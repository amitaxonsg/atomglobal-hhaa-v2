...
function AdminLogin({ onLogin }) {
  return <main className="admin-login"><div className="admin-login__card">
    <img src="/atom-global-logo.jpg" alt="Atom Global Consulting" />
...
export default function AdminApp() {
  ...
  return <div className="admin-shell"><aside className="admin-sidebar"><a href="/" className="admin-brand"><img src="/atom-global-logo.jpg" alt="Atom Global Consulting"/><span>Assessment CMS</span></a><nav>{sections.map(section => <button className={section === active ? "active" : ""} onClick={() => setActive(section)} key={section}>{section}</button>)}</nav><div className="admin-user"><i>AO</i><span>Alex Morgan<small>Owner · Preview</small></span></div></aside><main className="admin-main">{active === "Dashboard" && data && <Dashboard data={data} />}{active === "Content" && <ContentManager />}{!['Dashboard','Content'].includes(active) && <PlaceholderPage section={active} />}</main></div>;
}
