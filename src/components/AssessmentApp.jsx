...
function StageShell({ stageKey, current, total = 4, label, onBack, children, actions }) {
  ...
  return <div className="assessment-shell">
    <aside className="visual-panel" style={{ "--stage-image": `url(${stage.image})`, "--focal-point": stage.focalPoint, "--overlay": stage.overlay / 100 }} aria-label={stage.alt}>
      <img className="visual-logo" src="/media/brand/atom-global-wordmark.png?v=20260719-direct-1" alt="Atom Global Consulting" />
    ...
    <section className="application-panel"><header className="mobile-header"><img src="/media/brand/atom-global-wordmark.png?v=20260719-direct-1" alt="Atom Global Consulting"/><Progress current={current} total={total} label={label}/></header><div className="desktop-topbar"><button className="back-link" onClick={onBack} disabled={!onBack}><ArrowLeft/> Back</button><Progress current={current} total={total} label={label}/></div><main className="screen-content">{children}</main>{actions && <footer className="action-bar">{actions}</footer>}</section>
  </div>;
}
