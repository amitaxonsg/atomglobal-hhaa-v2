...
final class PdfService
{
    ...
    public function generate(int $reportId): string
    {
        ...
        $html = '<!doctype html><html><head><meta charset="utf-8"><style>'
            ...
            . '<div class="hero"><div class="score">' . (int) ($free['total'] ?? 0) . ' / 250</div><p>' . $this->h((string) (($free['summary']['summary'] ?? $free['summary'] ?? ''))) . '</p></div>'
    ...
    private function renderContent(mixed $content): string
    {
        ...
        foreach ($content as $key => $value) {
            ...
            if (is_array($value)) {
                if (array_is_list($value) && count(array_filter($value, 'is_scalar')) === count($value)) $html .= '<ul>' . implode('', array_map(fn($item) => '<li>' . $this->h((string) $item) . '</li>', $value)) . '</ul>';
                else $html .= '<pre>' . $this->h(json_encode($value, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE)) . '</pre>';
            } else $html .= '<p>' . $this->h((string) $value) . '</p>';
        }
        return $html;
    }
