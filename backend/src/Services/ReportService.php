...
final class ReportService
{
    ...
    public function generate(int $sessionId, int $scoreId, array $score, array $snapshot): array
    {
        ...
        $free = [
            ...
            'total' => $score['total'],
            'summary' => json_decode($profile['free_content_json'], true),
            'subscales' => $score['subscales'],
        ];
        $paid = [
            'profile' => $profile['profile_name'],
            'total' => $score['total'],
            'content' => json_decode($profile['paid_content_json'], true),
            'subscales' => $score['subscales'],
        ];
        $id = $this->db->insert(
    ...
    {
        if (!preg_match('/^[a-f0-9]{64}$/', $token)) return null;
        $row = $this->db->fetch('SELECT pdf_path FROM generated_reports WHERE secure_token_hash = ? AND revoked_at IS NULL AND token_expires_at > NOW() AND is_unlocked = 1 AND pdf_path IS NOT NULL', [hash('sha256', $token)]);
