...
foreach ($data['tracks'] as $track) {
    $container['db']->transaction(function () use ($container, $track, $data, $prices) {
        $existing = $container['db']->fetch('SELECT id FROM assessment_tracks WHERE track_key = ?', [$track['key']]);
        $trackId = $existing ? (int) $existing['id'] : $container['db']->insert('INSERT INTO assessment_tracks (track_key, name, description, price_minor, currency, is_active, display_order, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 1, ?, NOW(), NOW())', [$track['key'], $track['label'], $track['tagline'], $prices[$track['key']], 'USD', array_search($track['key'], ['personal','newjoiner','manager','executive'], true) + 1]);
        ...
        foreach ($track['answerChoices'] as $index => $label) $container['db']->execute('INSERT INTO answer_options (assessment_version_id, option_value, label, display_order) VALUES (?, ?, ?, ?)', [$versionId, $index + 1, $label, $index + 1]);
        $position = 1;
