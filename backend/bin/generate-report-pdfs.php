...
$rows = $db->fetchAll('SELECT id FROM generated_reports WHERE pdf_path IS NULL AND revoked_at IS NULL ORDER BY created_at LIMIT ' . $limit);

foreach ($rows as $row) {
    try {
        $path = $pdf->generate((int) $row['id']);
        echo 'Generated report ' . $row['id'] . ': ' . $path . "\n";
